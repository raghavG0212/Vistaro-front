...
export default function CreateSportPage({ onClose }) {
  ...
  const [sportSpecific, setSportSpecific] = useState({ sportType: "", team1: "", team2: "", matchFormat: "" });
  ...
  const setSportField = (k, v) => setSportSpecific((p) => ({ ...p, [k]: v }));
  ...
  const handleCreate = async () => {
    try {
      ...
      const slotPayload = {
        ...
        language: sportSpecific.matchFormat || "",
      ...
      console.error(err);
      alert("Something went wrong while creating sport event");
    }
  };

  return (
    <>
      <ModalHeader>Create Sport</ModalHeader>
      <ModalCloseButton />
      <ModalBody>
        ...
        <Accordion index={activeIndex} allowToggle>
          ...
          <AccordionItem>
            ...
            <AccordionPanel pb={4}>
              <VStack spacing={3} align="stretch">
                <Input placeholder="Sport Type (e.g. Cricket)" bg="gray.700" value={sportSpecific.sportType} onChange={(e) => setSportField("sportType", e.target.value)} />
                <Input placeholder="Team 1" bg="gray.700" value={sportSpecific.team1} onChange={(e) => setSportField("team1", e.target.value)} />
                <Input placeholder="Team 2" bg="gray.700" value={sportSpecific.team2} onChange={(e) => setSportField("team2", e.target.value)} />
                <Input placeholder="Match Format" bg="gray.700" value={sportSpecific.matchFormat} onChange={(e) => setSportField("matchFormat", e.target.value)} />
              </VStack>
            </AccordionPanel>
          </AccordionItem>
